...
impl GBuffer {
	...
	pub fn new(display: &Display, size: (u32, u32)) -> Self {
		...
		let emission_texture = Texture2d::empty_with_format(display, UncompressedFloatFormat::U8U8U8U8, MipmapsOption::NoMipmap, size.0, size.1).unwrap();
	...
	pub fn framebuffer(&self, display: &Display) -> MultiOutputFrameBuffer {
		let attachments = [
			...
			("o_emission", &self.emission_texture),
		];
	...
	pub fn emission_texture(&self) -> &Texture2d {
